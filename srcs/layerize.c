...
/*   Updated: 2025/03/26 12:40:17 by vcastald         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	unclosed_parenth(t_lexing *node)
{
	...
	while (tmp)
	{
		if (!ft_strncmp(node->type, "close_parenthesis", 18))
			return (1);
		tmp = tmp->next;
	}
	return (0);
...
int	check_parenthesis(t_lexing *lst, t_gen *gen)
{
	...
	while (tmp)
	{
		if (tmp->next)
			succ = tmp->next;
		if (!check_not_opened(tmp, lst))
			return (error_exit(gen, "minishell: syntax error", 2), 0);
		if (!ft_strncmp(tmp->type, "open_parenthesis", 17))
		{
			if (!check_close(tmp, succ))
				return (error_exit(gen, "minishell: syntax error", 2), 0);
			if (!unclosed_parenth(tmp))
				return (error_exit(gen, "minishell: syntax error", 2), 0);
